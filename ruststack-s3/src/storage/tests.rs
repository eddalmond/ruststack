...
mod object_tests {
    ...
    async fn test_delete_object_not_exists() {
        ...
        let result = s
            .delete_object("bucket", "nonexistent", None)
            .await
            .unwrap();
        assert!(!result.deleted);
    }
    ...
    async fn test_etag_consistency() {
        ...
        let result1 = s
            .put_object("bucket", "key1", data.clone(), ObjectMetadata::default())
            .await
            .unwrap();
        let result2 = s
            .put_object("bucket", "key2", data, ObjectMetadata::default())
            .await
            .unwrap();
...
mod object_edge_cases {
    ...
    async fn test_empty_object() {
        ...
        s.put_object("bucket", "empty", Bytes::new(), ObjectMetadata::default())
            .await
            .unwrap();
...
mod metadata_tests {
    ...
    async fn test_user_metadata() {
        ...
        meta.user_metadata
            .insert("another".to_string(), "value".to_string());
        ...
        let obj = s.get_object("bucket", "key", None).await.unwrap();
        assert_eq!(
            obj.metadata.user_metadata.get("custom-header"),
            Some(&"custom-value".to_string())
        );
        assert_eq!(
            obj.metadata.user_metadata.get("another"),
    ...
    async fn test_all_standard_metadata_headers() {
        ...
        assert_eq!(
            obj.metadata.content_disposition,
            Some("attachment; filename=\"test.txt\"".to_string())
        );
...
mod list_objects_tests {
    ...
    async fn test_list_empty_bucket() {
        ...
        let result = s
            .list_objects("bucket", None, None, None, 1000)
            .await
            .unwrap();
    ...
    async fn test_list_with_prefix() {
        ...
        let result = s
            .list_objects("bucket", Some("photos/2021/"), None, None, 1000)
            .await
            .unwrap();
        assert_eq!(result.objects.len(), 2);

        let result = s
            .list_objects("bucket", Some("photos/"), None, None, 1000)
            .await
            .unwrap();
        assert_eq!(result.objects.len(), 3);

        let result = s
            .list_objects("bucket", Some("videos/"), None, None, 1000)
            .await
            .unwrap();
    ...
    async fn test_list_with_delimiter() {
        ...
        let result = s
            .list_objects("bucket", None, Some("/"), None, 1000)
            .await
            .unwrap();
    ...
    async fn test_list_with_prefix_and_delimiter() {
        ...
        let result = s
            .list_objects("bucket", Some("photos/"), Some("/"), None, 1000)
            .await
            .unwrap();
    ...
    async fn test_list_alphabetical_order() {
        ...
        s.put_object("bucket", "c", Bytes::from("d"), ObjectMetadata::default())
            .await
            .unwrap();
        s.put_object("bucket", "a", Bytes::from("d"), ObjectMetadata::default())
            .await
            .unwrap();
        s.put_object("bucket", "b", Bytes::from("d"), ObjectMetadata::default())
            .await
            .unwrap();
...
mod copy_object_tests {
    ...
    async fn test_copy_same_bucket() {
        ...
        .await
        .unwrap();
        s.copy_object("bucket", "source", "bucket", "dest")
            .await
            .unwrap();
    ...
    async fn test_copy_different_buckets() {
        ...
        .await
        .unwrap();
        s.copy_object("source-bucket", "key", "dest-bucket", "key")
            .await
            .unwrap();
    ...
    async fn test_copy_preserves_metadata() {
        ...
        s.put_object("bucket", "source", Bytes::from("data"), meta)
            .await
            .unwrap();
        s.copy_object("bucket", "source", "bucket", "dest")
            .await
        ...
        assert_eq!(obj.metadata.content_type, Some("text/plain".to_string()));
        assert_eq!(
            obj.metadata.user_metadata.get("custom"),
            Some(&"value".to_string())
        );
    }
...
mod multipart_tests {
    ...
    async fn test_create_multipart_bucket_not_found() {
        let s = storage();
        let result = s
            .create_multipart_upload("nonexistent", "key", ObjectMetadata::default())
            .await;
        assert!(matches!(result, Err(StorageError::BucketNotFound(_))));
    }
    ...
    async fn test_upload_single_part() {
        ...
        let upload_id = s
            ...
            .unwrap();
        let part = s
            .upload_part("bucket", "key", &upload_id, 1, Bytes::from("part data"))
            .await
            .unwrap();
    ...
    async fn test_upload_multiple_parts() {
        ...
        let part1 = s
            .upload_part("bucket", "key", &upload_id, 1, Bytes::from("part1"))
            .await
            .unwrap();
        let part2 = s
            .upload_part("bucket", "key", &upload_id, 2, Bytes::from("part2"))
            .await
            .unwrap();
        let part3 = s
            .upload_part("bucket", "key", &upload_id, 3, Bytes::from("part3"))
            .await
            .unwrap();
    ...
    async fn test_multipart_out_of_order_upload() {
        ...
        let part3 = s
            .upload_part("bucket", "key", &upload_id, 3, Bytes::from("C"))
            .await
            .unwrap();
        let part1 = s
            .upload_part("bucket", "key", &upload_id, 1, Bytes::from("A"))
            .await
            .unwrap();
        let part2 = s
            .upload_part("bucket", "key", &upload_id, 2, Bytes::from("B"))
            .await
            .unwrap();
    ...
    async fn test_abort_multipart_upload() {
        ...
        let upload_id = s
            ...
            .await
            .unwrap();
        s.upload_part("bucket", "key", &upload_id, 1, Bytes::from("data"))
            .await
            .unwrap();

        s.abort_multipart_upload("bucket", "key", &upload_id)
            .await
            .unwrap();
    ...
    async fn test_upload_part_invalid_upload_id() {
        ...
        let result = s
            .upload_part("bucket", "key", "invalid-id", 1, Bytes::from("data"))
            .await;
    ...
    async fn test_complete_invalid_upload_id() {
        ...
        let result = s
            .complete_multipart_upload("bucket", "key", "invalid-id", vec![])
            .await;
        assert!(matches!(result, Err(StorageError::UploadNotFound(_))));
    }
    ...
    async fn test_multipart_with_metadata() {
        ...
        let upload_id = s
            ...
            .unwrap();
        let part = s
            .upload_part("bucket", "key", &upload_id, 1, Bytes::from("video data"))
            .await
            .unwrap();
        ...
        assert_eq!(
            obj.metadata.user_metadata.get("title"),
    ...
    async fn test_multiple_concurrent_uploads() {
        ...
        let upload1 = s
            .create_multipart_upload("bucket", "file1", ObjectMetadata::default())
            .await
            .unwrap();
        let upload2 = s
            .create_multipart_upload("bucket", "file2", ObjectMetadata::default())
            .await
            .unwrap();

        let part1a = s
            .upload_part("bucket", "file1", &upload1, 1, Bytes::from("1A"))
            .await
            .unwrap();
        let part2a = s
            .upload_part("bucket", "file2", &upload2, 1, Bytes::from("2A"))
            .await
            .unwrap();
        let part1b = s
            .upload_part("bucket", "file1", &upload1, 2, Bytes::from("1B"))
            .await
            .unwrap();
        let part2b = s
            .upload_part("bucket", "file2", &upload2, 2, Bytes::from("2B"))
            .await
            .unwrap();
