...
pub fn parse_key_condition(
    expression: &str,
) -> Result<Vec<KeyCondition>, ExpressionError> {
...
fn parse_single_key_condition(expr: &str) -> Result<KeyCondition, ExpressionError> {
    ...
    // Check for BETWEEN
    let between_re = regex_lite::Regex::new(
        r"(?i)^([#\w]+)\s+BETWEEN\s+([:\w]+)\s+AND\s+([:\w]+)$"
    ).unwrap();
    ...
    let comp_re = regex_lite::Regex::new(
        r"^([#\w]+)\s*(=|<>|<=|>=|<|>)\s*([:\w]+)$"
    ).unwrap();
    ...
    Err(ExpressionError::Invalid(format!(
        "Could not parse key condition: {}", expr
    )))
}
...
fn parse_condition_expr(expr: &str) -> Result<ConditionOp, ExpressionError> {
    ...
    // IN operator
    let in_re = regex_lite::Regex::new(
        r"(?i)^([#\w.]+)\s+IN\s*\(([^)]+)\)$"
    ).unwrap();
    ...
    Err(ExpressionError::Invalid(format!(
        "Could not parse condition: {}", expr
    )))
}

/// Parse an update expression
pub fn parse_update_expression(expression: &str) -> Result<ParsedUpdateExpression, ExpressionError> {
    ...
    for (clause_type, clause_body) in clauses {
        match clause_type.to_uppercase().as_str() {
            ...
            _ => return Err(ExpressionError::Invalid(
                format!("Unknown update clause: {}", clause_type)
            )),
        }
    }
...
fn parse_add_clause(clause: &str) -> Result<Vec<UpdateAction>, ExpressionError> {
    ...
    for part in parts {
        let part = part.trim();
        // Format: path value
        let space_pos = part.find(|c: char| c.is_whitespace())
            .ok_or_else(|| ExpressionError::Invalid(
                format!("Invalid ADD action: {}", part)
            ))?;
...
fn parse_delete_clause(clause: &str) -> Result<Vec<UpdateAction>, ExpressionError> {
    ...
    for part in parts {
        ...
        let space_pos = part.find(|c: char| c.is_whitespace())
            .ok_or_else(|| ExpressionError::Invalid(
                format!("Invalid DELETE action: {}", part)
            ))?;
        let path = part[..space_pos].trim().to_string();
        let value = part[space_pos..].trim().to_string();
...
pub fn evaluate_condition(
    ...
    match condition {
        ConditionOp::Comparison { path, op, values } => {
            ...
            match op {
                ComparisonOp::Eq => {
                    ...
                    Ok(item_value.map(|v| compare_attribute_values(v, expected) == std::cmp::Ordering::Equal).unwrap_or(false))
                }
                ComparisonOp::Ne => {
                    let expected = ctx.resolve_value(&values[0])?;
                    Ok(item_value.map(|v| compare_attribute_values(v, expected) != std::cmp::Ordering::Equal).unwrap_or(true))
                }
                ComparisonOp::Lt => {
                    let expected = ctx.resolve_value(&values[0])?;
                    Ok(item_value.map(|v| compare_attribute_values(v, expected) == std::cmp::Ordering::Less).unwrap_or(false))
                }
                ComparisonOp::Le => {
                    let expected = ctx.resolve_value(&values[0])?;
                    Ok(item_value.map(|v| {
                ...
                ComparisonOp::Gt => {
                    let expected = ctx.resolve_value(&values[0])?;
                    Ok(item_value.map(|v| compare_attribute_values(v, expected) == std::cmp::Ordering::Greater).unwrap_or(false))
                }
                ComparisonOp::Ge => {
                    let expected = ctx.resolve_value(&values[0])?;
                    Ok(item_value.map(|v| {
                ...
                ComparisonOp::Between => {
                    ...
                    Ok(item_value.map(|v| {
                        ...
                        (cmp_low == std::cmp::Ordering::Greater || cmp_low == std::cmp::Ordering::Equal)
                            && (cmp_high == std::cmp::Ordering::Less || cmp_high == std::cmp::Ordering::Equal)
                    }).unwrap_or(false))
                }
                ComparisonOp::In => {
        ...
        ConditionOp::BeginsWith(path, value) => {
            ...
            if let Some(AttributeValue::S { S: item_str }) = get_nested_attribute(item, &resolved_path) {
            ...
            if let Some(item_val) = get_nested_attribute(item, &resolved_path) {
                match (item_val, search) {
                    ...
                    (AttributeValue::L { L: list }, search_val) => {
                        for elem in list {
                            if compare_attribute_values(elem, search_val) == std::cmp::Ordering::Equal {
                                return Ok(true);
                            }
                        }
                    }
        ...
        ConditionOp::Not(inner) => {
            Ok(!evaluate_condition(inner, item, ctx)?)
        }
    }
}
        ...
        let matches = match &cond.op {
            ComparisonOp::Eq => {
                ...
                if cond.values.len() == 1 && cond.values[0].starts_with("BEGINS_WITH:") {
                    ...
                    if let (Some(AttributeValue::S { S: item_str }), AttributeValue::S { S: prefix_str }) = (item_value, prefix) {
                    ...
                    item_value.map(|v| compare_attribute_values(v, expected) == std::cmp::Ordering::Equal).unwrap_or(false)
                }
            }
            ComparisonOp::Lt => {
                let expected = ctx.resolve_value(&cond.values[0])?;
                item_value.map(|v| compare_attribute_values(v, expected) == std::cmp::Ordering::Less).unwrap_or(false)
            }
            ComparisonOp::Le => {
                let expected = ctx.resolve_value(&cond.values[0])?;
                item_value.map(|v| {
            ...
            ComparisonOp::Gt => {
                let expected = ctx.resolve_value(&cond.values[0])?;
                item_value.map(|v| compare_attribute_values(v, expected) == std::cmp::Ordering::Greater).unwrap_or(false)
            }
            ComparisonOp::Ge => {
                let expected = ctx.resolve_value(&cond.values[0])?;
                item_value.map(|v| {
        ...
        if let UpdateAction::Add { path, value } = action {
            ...
            match (item.get(&resolved_path), add_value) {
                ...
                (Some(AttributeValue::N { N: current }), AttributeValue::N { N: add }) => {
                    ...
                    item.insert(resolved_path, AttributeValue::N { 
...
fn evaluate_update_value(
    ...
    match value {
        UpdateValue::Value(placeholder) => {
            Ok(ctx.resolve_value(placeholder)?.clone())
        }
        ...
        UpdateValue::Plus(left, right) => {
            ...
            match (val1, val2) {
                (AttributeValue::N { N: n1 }, AttributeValue::N { N: n2 }) => {
                    ...
                    Ok(AttributeValue::N { N: (num1 + num2).to_string() })
                }
                _ => Err(ExpressionError::TypeMismatch),
            }
        }
...
fn split_by_and(expr: &str) -> Vec<&str> {
    ...
    while i < expr.len() {
        if bytes[i] == b'(' {
            ...
            if i + 3 <= expr.len() && &upper[i..i+3] == "AND" && 
    ...
    while i < expr.len() {
        if bytes[i] == b'(' {
            ...
            if i > 0 && bytes[i-1] == b' ' && 
               ...
               i + op.len() < expr.len() && bytes[i + op.len()] == b' ' {
...
mod tests {
    ...
    fn test_parse_update_expression() {
        let update = parse_update_expression("SET #name = :name, age = :age REMOVE old_field").unwrap();
    ...
    fn test_parse_update_with_if_not_exists() {
        let update = parse_update_expression("SET counter = if_not_exists(counter, :zero) + :inc").unwrap();
